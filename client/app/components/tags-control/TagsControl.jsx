...
function modelTagsControl({ archivedTooltip }) {
  ...
  function ModelTagsControl({ isDraft, isArchived, ...props }) {
    return (
      <TagsControl {...props}>
        {!isArchived && isDraft && <span className="label label-tag-unpublished">為公開</span>}
        {isArchived && (
          <Tooltip placement="right" title={archivedTooltip}>
            <span className="label label-tag-archived">歸檔</span>
          </Tooltip>
        )}
      </TagsControl>
    );
...
export const QueryTagsControl = modelTagsControl({
  archivedTooltip: "此查詢已歸檔，不能在儀表板中使用，也不能出現在搜尋結果中。",
});

export const DashboardTagsControl = modelTagsControl({
  archivedTooltip: "此儀表板以歸檔，不會在儀表板或搜尋結果中出現",
});
